...
function filter_jon(seq, pred)
{
	var offset = 0;
	var seq_length = seq.length;
	for (var i = 0, ie = seq.length; i < ie; ++i) {
	...
	if (offset > 0) {
		...
		for (var i = seq.length; i != seq_length; ++i) {
			...
			delete seq[i];
			av += (new Date).getTime() - s;
		}
	}
	console.log("fixing av:", av / (seq_length - seq.length));
	console.log("fixing", (new Date).getTime() - start);

	return seq;
}

function filter_js(seq, pred)
{
	var offset = 0;
	for (var i = 0, ie = seq.length; i < ie;) {
		if (pred.call(seq, seq[i])) {
			Array.prototype.splice.call(seq, i, 1);
			--ie;
		}
		else {
			++i;
...
var testrange = {}, testrange2 = [], testrange3 = [];
testrange.length = 0;
for (var i = 0; i != 114468; ++i) {
	testrange[i] = i;
	++testrange.length;
	testrange2.push(i);
	testrange3.push(i);
...
filter_jon(testrange, function(x) {
	return x % 2 == 0;
});
...
start = (new Date).getTime();
filter_js(testrange2, function(x) {
	return x % 2 == 0;
});
end = (new Date).getTime() - start;
console.log(end);
//console.log(testrange2);

start = (new Date).getTime();
testrange3 = testrange3.filter(function(x) {
	return x % 2 != 0;
});
end = (new Date).getTime() - start;
console.log(end);
//console.log(testrange3);
...
jam.filter(k2, function(x) {
	return x == 1;
}, {with_filtered: function(things) {
	console.log(things);
}});
...
(function(undefined) {
	...
	jam.Range = function() {
		if (this.constructor == jam.Array) {
			return this;
		}

		return jam.Range.init.apply(new jam.Range(), arguments);
	};

	$.extend(jam.Range, {
		init: function(data, begin, end) {
			$.extend(this, jam.Indexable, jam.Range);

			this.__proto__.constructor = jam.Array;
			this._data = data;
			this._begin = begin;
			this._end = end;
		},
...
(function(undefined) {
	jam.Array = function()
	{
		...
		return jam.Array.init.apply(new jam.Array(), arguments);
	};
	...
	$.extend(jam.Array, {
		...
		{
			$.extend(this, jam.Sequence, jam.Array);
