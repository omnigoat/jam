...
jam.define_class = function(jam_Object_name)
{
	...
	function _inheritor() {
		...
		for (var i = 0, ie = args.length; i != ie; ++i)
		{
			...
			for (var p in x) {
				if (p !== "init" && p.match(/^__(.+)__$/) === null && x.hasOwnProperty(p)) {
					jam_Object.prototype[p] = x[p];
				}
			}
	...
	return {
		inheriting: function() {
			...
			return {
				as: function() {
					_as.apply(jam_Object, arguments)
					return jam_Object;
				}
			};
		},
...
var A = jam.define_class().as({
	name: "A",
	init: function() { console.log("A: initing"); },
	talk: function() { console.log("A!"); },
	emote: function() { console.log("whaaaA!"); }
});

jam.Dragon = jam.define_class().inheriting(A).as({
	name: "B",
	emote: function() { console.log("WBBBBB"); },
	init: function() { console.log("B: initing"); }
});

jam.AwesomeDragon = jam.define_class().inheriting(jam.Dragon).as({
	name: "C",
	init: function() { console.log("C: initing"); }
});

console.log( jam.AwesomeDragon() );
