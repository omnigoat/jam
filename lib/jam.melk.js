	...
	$.extend(jam.melk, {
		...
		_render: function(stack, state)
		{
			...
			// string
				...
				// new child element
					...
					if (regex[1] === "$") {
						var ta = $(regex[5]);
						console.log("  appending: " + ta);
						stack.dom_parent.append(ta);
					}
					else {
		...
		_render_element: function(stack, state, regex) {
			...
			if (regex[5] !== undefined) {
				var html;
				try {
					html = $(regex[5])[0].innerHTML;
				}
				catch (e) {
					console.log("  problem: " + regex[5]);
					html = regex[5];
				}

				console.log("  rendering-element: " + html);
				e.html(html);
			}
