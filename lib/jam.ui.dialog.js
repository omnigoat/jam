...
(function(undefined) {
	...
	jam.ui.Dialog = jam.define_class({as: {
		...
		init: function(options)
		{
			...
			this._data = options.persist ? options.data : options.data.clone(true);
		...
		{
			var data_width = this.options.data.outerWidth(),
			    data_height = this.options.data.outerHeight(),
			...
			this._elements.container.css({
				top: $(document).height() / 2 - data_height / 2,
				left: $(document).width() / 2 - data_width / 2,
