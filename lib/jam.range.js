...
(function(undefined) {
	...
	jam.Range = jam.prototype({
		...
		statics: {
			is_forward: function(range) {
				...
				return P.hasOwnProperty("head") && P.hasOwnProperty("tail") && range.pop_head;
			},
	...
	jam.ArrayRange = jam.prototype({
		...
		methods: {
			at: function(i) {
				jam.assert(i >= 0 && i < (this._until - this._current), "jam.ArrayRange.at: bad index (" + i + ")");
		...
		method_missing: function(i) {
			i = +i;
			return isNaN(i) ? undefined : this.at(i);
		}
	});
	...
	jam.MapRange = jam.prototype({
		...
		init: function(range, fn)
		{
			...
			if (this._is_random_access)
			{
				...
				this._define_accessors({
					length: function() {
						return this._range.length;
					}
				});

				//jam.Range.mixins.RandomAccess(this);

				this._mixin(jam.RandomAccessRangeMixin);
			}
		...
		// MapRange is only a random-access range if its _range is,
		...
		method_missing: function(i)
		{
			...
			// define get-accessor for property, bind the property to keep up
			// to date with the index-offset.
			Object.defineProperty(this, i, {
				get: function() {
					console.log("index: " + (this._index_offset + i));
					if (this._cached_values[this._index_offset + i] === undefined) {
						this._cached_values[this._index_offset + i]
							= this._fn.call(this._range[i], this._range[i]);
					}

					return this._cached_values[this._index_offset + i];
				}
			});

			// we've created an accessor for this value, so call it.
			return this[i];
		}
	});
...
/*
	...
	//
	//=====================================================================
	jam.DeferredRange = jam.prototype({
	supers: [jam.Range],
	...
	accers: {
		head: function() {
			if (this._head === null)
				return this._head = this._get_head();
