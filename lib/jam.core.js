...
var jam = (function($, undefined) {
	...
	$.extend(jam.test, {
		...
		default_handler: function(message, stack) {
			if (stack !== undefined)
				console.log(message, [ stack[0].match(/\((.+)\)/)[0] ]);
			else
				console.log(message);
		},
		...
		prototype: function(definitions)
		{
			...
			if (definitions.supers)
			{
				...
				jam.for_each_depth_first(jam_Object, function() {
					...
					//console.log("ACCERS: " + definitions.accers + " vs ", this.prototype.__accers__);
			...
			*/
			...
			if (definitions.accers) {
				...
				jam.each(jam_Object.prototype.__accers__, function(k, v) {
					Object.defineProperty(jam_Object.prototype, k, {
						get: v instanceof Function ? v : v.get,
						set: v instanceof Function ? undefined : v.set,
						enumerable: k.match(/^_/) == null
...
protics: {
	always: function(fn) {
		this._alwaysers.push(fn);
	},

	success: function(fn) {
		this._successers.push(fn);
	},

	fail: function(fn) {
		this._failers.push(fn);
	},

	resolve: function() {
		var args = Array.prototype.slice.call(arguments);

		jam.each(this._successers.concat(this._alwaysers), function(fn) {
			fn.apply(fn, args);
		});
	},
	...
	reject: function() {
		...
		jam.each(this._failers.concat(this._alwaysers), function(fn) {
			fn.apply(fn, args);
		});
	}
}});
