...
var jam = (function(undefined) {
	...
	//=====================================================================
	jam.extend(jam, {
		assert: function(v, m, handler) {
			if (v === false) {
				(handler ? handler : jam.assert.handler)(m ? m : "Jam: an error occurred!");
			}
		}
	});

	jam.extend(jam.assert, {
		throw_handler: function(m) {
			throw new Error(m);
		},

		console_error_handler: function(m) {
			console.error(m);
		}
	});

	jam.assert.handler = jam.assert.throw_handler;
	...
	jam.extend(jam, {
		...
		curry: function()
		{
			...
			return function()
			{
				...
				args.splice(0, placeholders + undefineds);
				jam.each(args, function(x) {
					final_args.push(x);
				});
	...
	jam.extend(jam, {
		prototype: function(definitions)
		{
			...
			//---------------------------------------------------------------------
			definitions = jam.extend({accessors:{}, methods:{}}, definitions);
			
			jam.each_in_object(definitions.accessors, function(accessor, name, accessors) {
				if (!(accessor instanceof Function) && accessor.pattern !== undefined) {
					definitions.dynamic_accessors = definitions.dynamic_accessors || {};
					accessor.set = accessor.set || function() {};
					accessor.get = accessor.get || function() {};
					definitions.dynamic_accessors[name] = accessor;
					delete accessors[name];
				}
			});

			jam.each_in_object(definitions.methods, function(method, name, methods) {
				if (!(method instanceof Function) && method.pattern !== undefined) {
					definitions.dynamic_methods = definitions.dynamic_methods || {};
					definitions.dynamic_methods[name] = method;
					delete methods[name];
				}
			});
			...
			function jam_Object() {
				...
				if ("Proxy" in window && (definitions.dynamic_accessors || definitions.dynamic_methods || definitions.proxy)) {
					definitions.proxy = definitions.proxy || {};
					...
					obj = jam.proxies.forwarder(obj,
					{
						get: function(receiver, name)
						{
							if (definitions.proxy.get)
								definitions.proxy.get.call(receiver, name);
							...
							// accessors first
							jam.each(definitions.dynamic_accessors, function(accessor, k) {
								var regex = name.match(accessor.pattern);
								if (regex != null) {
									result = accessor.get.call(receiver, regex);
									return false;
								}
							});
							...
							// next, methods
							jam.each(definitions.dynamic_methods, function(method, k) {
								var regex = name.match(method.pattern);
								if (regexd != null) {
									result = method.apply;
								}
							});
							...
							// try __method_missing__
							if (definitions.methods.__missing__) {
								result = definitions.methods.__missing__.call(receiver, name);
								if (result !== undefined)
									return result;
							}
							...
							// just forward on
							return jam_Object.prototype[name];
						},

						set: function(receiver, name, value) {
							if (definitions.proxy.set) {
								definitions.proxy.set.call(receiver, name, value);
							}

							// accessors first
							jam.each(definitions.dynamic_accessors, {
								apply: function(accessor, k) {
									var regex = name.match(accessor.pattern);
									if (regex != null) {
										accessor.set.call(receiver, regex, value);
										return false;
									}
								},
						...
						has: function(name) {
							definitions.proxy.has && definitions.proxy.has.call(this, name);

							return jam.if_any(definitions.dynamic_accessors, function(x) {
									return name.match(x.pattern) != null;
								})
								||
								jam.if_any(definitions.dynamic_methods, function(x) {
									return name.match(x.pattern) != null;
								})
								||
								name in target;
	...
	jam.extend(jam, {
		...
		// reindex
		// ---------
		//   given an array, re-calculates an index (if negative), to
		//   index from the end. -1 is the last element in the array, -2
		//   the second-last, etc.
		//---------------------------------------------------------------------
		reindex: function(range, i) {
		...
		each: function(range, this_object, fns, definitely_is_array)
		{
			...
			if (definitely_is_array === undefined) {
				delegate = jam.is_arraylike(range) ? jam._each_in_array : jam._each_in_object;
			}
...
(function(undefined) {
	...
	jam.test("jam.each()",
		...
		{should: "iterate correctly", ie: function(a) {
			var result = [];

			jam.each(a, function(x, i) {
				result.push(x);
			});
		...
		{should: "pass the correct index along", ie: function(a) {
			var indexes = [];
			jam.each(a, function(x, i) {
				indexes.push(i);
			});
