...
var jam = (function($, undefined) {
			...
			for (var i = 1, ie = arguments.length; i != ie; ++i )
			{
				...
				if ( (src = arguments[i]) != null ) {
					...
					for (name in src)
					{
						...
						if (target !== element && element !== undefined) {
							target[name] = element;
						}
					}
				}
	...
	jam.extend(jam, {
		...
		{
			//---------------------------------------------------------------------
			// morph definitions into something easier to use
			//---------------------------------------------------------------------
			definitions = jam.extend({methods:{}}, definitions);
			
			for (var p in definitions.methods) {
				if (!(definitions.methods[p] instanceof Function)) {
					definitions.dythods = definitions.dythods || {};
					definitions.dythods[p] = definitions.methods[p];
					delete definitions.methods[p];
				}
			}
			...
			function jam_Object() {
				...
				if ("Proxy" in window && definitions.dythods) {
					var target = obj;
					obj = jam.proxies.forwarder(obj, {
						get: function(receiver, name) {
							var matches = [];

							jam.each(definitions.dythods, function(dythod, k) {
								jam.assert(dythod.match, "jam.prototype.get: pattern has no match property");
								var regex = name.match(dythod.pattern);
								if (regex != null) {
									matches.push(jam.extend({name: k, call_name: name, regex: regex}, dythod));
									if (dythod.rewrite)
										name = dythod.rewrite;
								}
							});
							...
							//------------------------------------------------------
							// create a chain of curried functions
							//------------------------------------------------------
							var fn = function() {
								if (name in target)
							...
							jam.each(matches, {reverse: true}, function(x) {
								var old_fn = fn;
								fn = function()
								{
									var args = Array.prototype.slice.apply(arguments);
									
									var parameters = jam.map(x.parameters, function(parameter)
									{
										if (typeof parameter == "string" && parameter.match(/^\$(\d+)$/)) {
											return x.regex[parameter.replace(/\$(\d+)/, "$1")];
										}
										else if (parameter instanceof jam.placeholder) {
											if (parameter.value === undefined) {
												return args[i++];
											}
											else {
												++i;
												return args[parameter.value];
											}
										}
									});

									x.perform && x.perform.apply(this, [x.regex].concat(parameters));
									old_fn.apply(this, parameters);
								}
								
								// short out for operators
								if (x.name.match(/_operator$/) != null) {
									operator = fn(x.parameters);
									return false;
								}
							});

							return (operator !== undef) ? operator : fn;
						},
					}, jam_Object.prototype);
				}

				jam_Object.prototype.__init__ && 
					jam_Object.prototype.__init__.apply(obj, arguments);
			...
			// superclass
			//---------------------------------------------------------------------
			var supr = (definitions.super === undefined) ? jam.BaseObject : definitions.super;
			

			//---------------------------------------------------------------------
			// jam_Object.prototype must be overridden correctly
			//---------------------------------------------------------------------
			if ((definitions.methods.__missing__ || definitions.methods.__patterns__) && "Proxy" in window) {
				jam_Object.prototype = Object.create(
					jam.proxies.forwarder(supr.prototype, {
						get: function(receiver, name)
						{
							jam.assert(name != "__method_missing__", "jam_Object: __method_missing__ not present");

							var mods = apply_patterns.call(
							    	receiver,
							    	definitions.methods.__patterns__,
							    	name,
							...
							// if we rewrote the name, then assuming we have a matching method, we'll
							// curry the function
							if (name in receiver) {
								return jam.curry.apply(null, [receiver[name]].concat(mods.params));
							}

							// __method_missing__
							if (definitions.methods.__missing__) {
								var result = receiver.__method_missing__(name);
								if (result !== undefined) {
									return result;
								}
							}

							// __method_missing__ didn't work, next shop around in our superclass
							if (name in supr.prototype) {
				...
				Object.defineProperty(jam_Object.prototype, "__method_missing__", {value: definitions.methods.__missing__});
			}
			else if (supr === jam.BaseObject || supr.prototype instanceof jam.BaseObject) {
				jam_Object.prototype = Object.create(
					supr.prototype,
			...
			// accessors
			//---------------------------------------------------------------------
			jam.each(definitions.accessors, function(v, k) {
			...
			// methods
			//---------------------------------------------------------------------
			jam.each(definitions.methods, function(v, k) {
				if (k.match(/^__/))
					return;
			...
			// static methods
			//---------------------------------------------------------------------
			jam.each(definitions.statics, function(v, k) {
				jam_Object[k] = v;
			});
			...
			if (old_init) {
				Object.defineProperty(jam_Object.prototype, "__init__", {
					value: function()
					{
						...
						if (supr.prototype.__init__)
							args.unshift(supr.__init__);
						
						old_init && old_init.apply(this, args);

						if (!("Proxy" in window) && definitions.__no_proxies__) {
							definitions.__no_proxies__.call(this);
						}
					}
				});
			}
	...
	jam.BaseObject.prototype = {
		constructor: jam.BaseObject,

		_define_member: function(k, v) {
			k = k.toString();
		...
		_define_members: function(map) {
			jam.each(map, {this_object: this}, jam.curry(this._define_member, _2, _1));
		},

		_define_accessor: function(k, v) {
			k = k.toString();
		...
		_define_accessors: function(map) {
			jam.each(map, {this_object: this}, jam.curry(this._define_accessor, _2, _1));
		}
	};

	function apply_patterns(patterns, name, params)
	{
		var rewrote = false;

		// see if the method matches any of our patterns
		jam.each(patterns, {this_object: this}, function(pattern)
		{
			var regex = name.match(pattern.match);

			if (regex != null && (pattern.partial || regex[0].length === name.length))
			{
				if (pattern.params) {
					var i = 0;
					params = jam.map(pattern.params, function(x) {
						if (typeof x == "string" && x.match(/\$(\d+)/)) {
							return regex[x.replace(/\$(\d+)/, "$1")];
						}
						else if (x instanceof jam.placeholder) {
							...
							jam.assert(false, "HELL NO");
						}
					});
				}

				if (pattern.apply) {
					pattern.apply.call(this, regex);
				}

				if (pattern.rewrite) {
					name = name.replace(pattern.match, pattern.rewrite);
					rewrote = true;
				}
			}
		});

		return {rewrote: rewrote, name: name, params: params};
	}
	...
	jam.extend(jam, {
		...
		{
			options = jam.extend(jam._default_range_options(range), options);
			...
			if (options.from == 0 && options.until == range.length)
			{
				Array.prototype.forEach.call(range, function(x, i) {
					if (i !== 0 && callbacks.between) {
						callbacks.between.call(options.this_object || range, range[i - 1], x, i, range);
					}

					if (options.applicator(fn, options.this_object || range, [x, i, range]) === false) {
		...
		_each_in_object: function(range, options, fn, callbacks)
		{
			...
			for (var p in range) {
				...
				result = options.applicator(fn, options.this_object || {key: p, value: range[p]}, [range[p], p, range]);
			}
			return result;
		},
		...
		_filter: function(seq, options, pred, callbacks)
		{
			...
			if (Array.isArray(seq) && options.from == 0 && options.until == seq.length && Array.prototype.filter) {
				Array.prototype.filter.call(seq, pred, options.this_object);
			}
		...
		// fold
		//---------------------------------------------------------------------
		fold: function(xs, init, f) {
			return jam._fold(xs, init, f);
		},

		_fold: function(xs, init, fn)
		{
