...
var jam = (function(undefined) {
	...
	var jam =
	{
		...
		define_member: function(object, k, v, o) {
			v = v !== undefined ? v : o !== undefined ? o.value : undefined;
		...
		each: function(range, this_object, fns, definitely_is_array)
		{
			...
			if (result.completed && fns.completed) {
				return fns.completed.call(this_object, range) || result.value;
			}
			else if (!result.completed && fns.broke) {
				return fns.broke.call(this_object, result.value, range[result.index], result.index, range) || result.value;
			}
		...
		_each_in_object: function(range, this_object, fn, betweener)
		{
			try {
				var property, result, i = 0, prev;
				for (var property in range)
				{
					if (i++ !== 0 && betweener) {
						betweener.call(this_object, prev, range[property], range);
					}
