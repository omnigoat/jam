	...
	$.extend(jam, {
		prototype: function(definitions)
		{
			function jam_Object() {
				...
				jam_Object.prototype.__jam_init__.apply(self, arguments);
				return self;
			};
			
			
			jam_Object.prototype.__jam_init__ = function()
			{
				// call all our supers
				if (jam_Object.prototype.__supers__) {
					jam.each(jam_Object.prototype.__supers__, {this_object: this}, function(s) {
						s.call(this);
					});
				}
				
				// call our __init__, guaranteed to exist
			...
			if (definitions.supers) {
				var supers = [];
				// for each superclass
				jam.each(definitions.supers, function(x) {
					// if that superclass has superclasses (ultrasuperclasses?), add them first
					if (x.prototype.__supers__)
						supers = supers.concat(x.prototype.__supers__);
					// then add the superclass
					x.prototype.__init__ && supers.push(x.prototype.__init__);
				});
				jam_Object.prototype.__supers__ = supers;
			}

			if (definitions.datas) {
				jam_Object.prototype.__datas__ = definitions.datas;
			}
			...
			if (definitions.protics) {
				for (var x in definitions.protics) {
					jam_Object.prototype[x] = definitions.protics[x];
				}
			}
			...
			// rearrange __init__ to always apply datas before anything else. this
			// means that when we're called as a super, we'll apply our own datas
			// first. good stuff.
			var old_init = definitions.protics && definitions.protics.__init__;
			
			jam_Object.prototype.__init__ = function()
			{
				if (jam_Object.prototype.__datas__) {
					jam.each(jam_Object.prototype.__datas__, {this_object: this}, function(k, v) {
						this[k] = v;
					});
				}
				...
				try {
					Array.prototype.forEach.call(range, function(x, i, a) {
						...
						if (options.applicator(fn, options.this_object || x, [x, i, a]) === false) {
							result = false;
							throw jam.StopIterationException;
						}
					});
				}
				catch (e) {
					if (e !== jam.StopIterationException)
						throw e;
				}
			}
			else {
...
jam.BakeableRange = jam.prototype({
datas: {
	_require_baking: true
},
...
protics: {
	...
	require_baking: function() {
		return this._require_baking;
	}
}});

jam.OtherRange = jam.prototype({
supers: [jam.BakeableRange]
});

var BR = jam.BakeableRange();
var OR = jam.OtherRange();
