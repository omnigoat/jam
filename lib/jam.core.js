...
var jam = (function($, undefined) {
	...
	$.extend(jam, {
		prototype: function(definitions)
		{
			...
			// store the superclasses for later initialisation, and then steal all
			// their protics
			if (definitions.supers)
			{
				jam_Object.prototype.__supers__ = definitions.supers;
				
				jam.for_each_depth_first(jam_Object, function() {
					for (var p in this.prototype) {
						if (this.prototype.hasOwnProperty(p) && p.match(/^__(.+)__$/) === null) {
							jam_Object.prototype[p] = this.prototype[p];
						}
					}
				},
				function() {
			...
			jam_Object.prototype.__init__ = function()
			{
				...
				jam.each(jam_Object.prototype.__supers__, function() {
					this.prototype.__init__ && this.prototype.__init__.apply(self, arguments);
				});
