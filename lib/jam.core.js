...
var jam = (function(undefined) {
	...
	var jam =
	{
		...
		define_member: function(object, k, o, v) {
			...
			jam.assert(v !== null, "jam.define_member: member can't be null!");
			k = k.toString();
		...
		define_members: function(object, members) {
			jam.each_in_object(members, jam.curry(jam.define_member, object, _2, _1));
		},
	...
	jam.extend(jam, {
		prototype: function(blueprint)
		{
			...
			jam.each_in_object(blueprint.accessors, function(accessor, name, accessors) {
				if (!(accessor instanceof Function) && accessor.pattern !== undefined) {
					blueprint.dynamic_accessors = blueprint.dynamic_accessors || {};
					accessor.set = accessor.set || function() {};
					accessor.get = accessor.get || function() {};
					blueprint.dynamic_accessors[name] = accessor;
					delete accessors[name];
				}
			});

			jam.each_in_object(blueprint.methods, function(method, name, methods) {
				if (!(method instanceof Function) && method.pattern !== undefined) {
					blueprint.dynamic_methods = blueprint.dynamic_methods || {};
					blueprint.dynamic_methods[name] = method;
					delete methods[name];
				}
			});
			...
			function jam_Object() {
				...
				if ("Proxy" in window && (blueprint.dynamic_accessors || blueprint.dynamic_methods || blueprint.proxy)) {
					blueprint.proxy = blueprint.proxy || {};
					...
					obj = jam.proxies.forwarder(obj,
					{
						get: function(receiver, name)
						{
							if (blueprint.proxy.get)
								blueprint.proxy.get.call(receiver, name);
							...
							// accessors first
							jam.each(blueprint.dynamic_accessors, function(accessor, k) {
							...
							// next, methods
							jam.each(blueprint.dynamic_methods, function(method, k) {
							...
							if (blueprint.methods.__missing__) {
								result = blueprint.methods.__missing__.call(receiver, name);
								if (result !== undefined)
									return result;
							}
						...
						set: function(receiver, name, value) {
							if (blueprint.proxy.set) {
								blueprint.proxy.set.call(receiver, name, value);
							}

							// accessors first
							jam.each(blueprint.dynamic_accessors, {
						...
						has: function(name) {
							blueprint.proxy.has && blueprint.proxy.has.call(this, name);

							return jam.if_any(blueprint.dynamic_accessors, function(x) {
									return name.match(x.pattern) != null;
								})
								||
								jam.if_any(blueprint.dynamic_methods, function(x) {
									return name.match(x.pattern) != null;
								})
								||
								name in target;
			...
			//---------------------------------------------------------------------
			// if ((blueprint.methods.__missing__ || blueprint.methods.__patterns__) && "Proxy" in window) {
			...
			// 	Object.defineProperty(jam_Object.prototype, "__method_missing__", {value: blueprint.methods.__missing__});
			// }
			// else
			...
			//---------------------------------------------------------------------
			// create properties upon the prototype and the class
			//---------------------------------------------------------------------
			// accessors
			jam.define_accessors(jam_Object.prototype, blueprint.accessors);
				...
				Object.defineProperty(jam_Object.prototype, "__init__", {
					value: function()
					{
						...
						if (supr.prototype.__init__)
							this.__super__ = supr.prototype.__init__;

						blueprint.constructor.apply(this, args);
