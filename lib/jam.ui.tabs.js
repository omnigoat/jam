...
(function(undefined){
	...
	$.extend(jam.ui.tabs, {
		init: function(elem, options)
		{
			...
			// });
			this.tabs_set = [].concat(this.$tabs.get());
			...
			this.tabs_topology = [];
			jam.each(this.$tabs, {this_object: this}, function(x, i) {
				this.tabs_topology[x] = {index: i, animating: false};
				this.tabs_topology[i] = x;
				$(x).css("z-index", this.$tabs.length - i - 1);
			});
			...
			this.$tabs.bind("mousedown", function(event)
			{
				...
				this.style.zIndex = widget.$tabs.length;
			...
			if (this._settings.close_button)
			{
				...
				this.$tabs.find("div.ui-tab-close-circle2")
					...
					.bind("mouseup", function(event) {
						...
						widget.tabs_set.sort(widget._tabs_set_pred);
						var $dom = $(this).parents(".ui-tab"),
						    dom = $dom[0]
						    index = widget.tabs_set.indexOf(dom),
						...
						$dom.animate({width: "toggle"}, 200, function() {
							$dom.remove();
							widget.tabs_set.splice(index, 1);
							widget.tabs_topology.splice(index, 1);
						});
					})
					;
			}
		...
		tab: function(id) {
			...
			return jam.extend({iq: tab}, tab_prototype);
		},


		_tabs_set_pred: function(lhs, rhs) {
			return $(lhs).position().left < $(rhs).position().right;
		},
		...
		_drag: function(widget)
		{
			...
			var drag_tab = this,
					$drag_tab = $(drag_tab),
			    drag_index = widget.tabs_set.indexOf(drag_tab),
