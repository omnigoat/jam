...
jam.ui.Dialog = jam.prototype({
	...
	constructor: function(options)
	{
		...
		this.define_members({
			...
			_data: options.persist ? options.data : options.data.clone(true, true),

			// if we are not persisting the data, we need the original data to clone
			// every time open() is called. otherwise, null is fine.
	...
	methods: {
		...
		{
			if (!this._options.persist) {
				this._data = this._original_data.clone(true, true);
				this._data.appendTo(this._elements.container);
			}
			...
			if (this._options.modal) {
				jam.assert(!jam.ui.Dialog._currently_modal, "jam.ui.Dialog.open: Modal dialog already open!");
				jam.ui.Dialog._currently_modal = true;
				return;
			}
		...
		_bind_events: function(apply_to_persist)
		{
			var self = this;
