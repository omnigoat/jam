...
;(function(jam, $, undefined) {
	...
	//=====================================================================
	jam.ui.esh = function self($elem, options) {
		return new jam.ui.esh._init($elem, options);
	};
	...
	$.extend(jam.ui.esh, {
		...
		{
			var vsb = this.$vsb.data('jam.ui.scrollbar');
		...
		_line_containing_point: function(point)
		{
			...
			return jam.binary_search(lines, function(line)
			{
				var $line = $(line),
						pos = $line.offset()
						;
		...
		set_line: function(n, highlight)
		{
			...
			this.$vsb.data('jam.ui.scrollbar').value(n);
		...
		_activate_line_by_line_selection: function(mousedown_line)
		{
			...
			.bind('mousemove.highlight', function(e)
			{
				...
				var to_line_index = -1;
				//console.log(jam.page.mousedown_position.y);
				if (e.pageY < jam.page.mousedown_position.y) {
		...
		_recalculate_selected_lines: function()
		{
			...
			if (!jam.arrays_equal(old_indices, all_indices)) {
		...
		_resize: function()
		{
			...
			jam.dynamically_size({
				...
				using: {
					height: function($whole) {return $whole.outerHeight() - this.position().top - jam.page.scrollbar_width;},
					width: function($whole) {return $whole.outerWidth() - this.position().left - jam.page.scrollbar_width;},
				},
			});

			this.$vsb.css({
				right: 0,
				width: jam.page.scrollbar_width,
			});
			...
			this.$hsb.css({
				right: jam.page.scrollbar_width,
				height: jam.page.scrollbar_width,
			});
		},
		...
		_init: function($sh, options)
		{
			...
			this.$parent.mousewheel(function(event, delta) {
				var $vsb = self.$vsb;
				$vsb.data('jam.ui.scrollbar').value(-delta, true, true);
